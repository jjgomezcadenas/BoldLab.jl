...
begin
	...
	pb ="5e4"
	dk ="1e5"
	rootdir = "/Users/jjgomezcadenas/Projects/BoldLab/pluto/npy/"
	mcfile = string("n3d_mc_", nmols, "_", power, "_pb_", pb, "_dk_",dk,"_noise.npy" )
	fpath = joinpath(rootdir, mcfile)
md"""
### Data set
 - MC, $(nmols) molecules in 30 x 30 microns, with $(power) power, photobleaching at $(pb) photons and dark transitions at $(dk) photons. 

- file to read, full path : $(fpath)	

"""
end

# ╔═╡ cb1051a9-e506-4c4a-939e-43ef54e384b8
begin
	#pathmc = "/Users/jjgomezcadenas/Projects/BoldLab/pluto/npy/n3d_mc_1e2_5mW_pb_1e5_dk_1e6_noise.npy"
	pathmc = fpath
	pedmc = 0.0
	md"""
	- Read the data and plot show a few frames
	"""
end
...
# ╔═╡ 94a3f219-6e40-4edb-a9f1-eb7e33f72294
pathmc

# ╔═╡ 1655b47b-fdf3-40f4-a49b-c679c9a956e5
function png_from_path(path)
	
	parts = split(path, '/')
	file = parts[end]      # Field1
	replace(file, ".npy" => ".png")
end

# ╔═╡ 52510ef2-ea5e-488b-b62a-bc2fdbcfe962
png_from_path(pathmc)
...
# ╔═╡ 72b4d07e-4836-45fd-8f11-d1bdc08e2836
begin
    @bind fitASF10 CheckBox(false)
end

# ╔═╡ 189964c8-32cb-49dd-9cbf-564026229d33
begin
    @bind fitASF3 CheckBox(false)
end

# ╔═╡ 9869bee1-6942-44bf-b69a-62b1a43c487d
begin
...
# ╔═╡ 8e4255af-23cd-4f26-8b6b-e0c24c93246c
png_from_path(pathmc)

# ╔═╡ 88352dd8-f5a2-4bd7-9a8b-b77d35e7698b
...
plot_traces(TRZS, peaks; ftrz=1, ltrz=14,  figsize=(1500,1500))
...
if fitPELT
plotsdf_pelt(pdf; ppath= joinpath("stepAnalysis", png_from_path(pathmc)))
end
...
- The algorithm can be run without parameters. The efficiency is higher than ASF (with niter = 3. The algorithm finds always 2 steps as expected in the data and the expected distributions in step hegith and time. 
"""
end
