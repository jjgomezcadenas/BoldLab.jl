...
begin
	...
	md"""
	- Load the stack:  $(length(mask[mask .>0])) pixels masked with signal > $(nsigma) times the mean of the first frame
	"""
end

# ╔═╡ 7707870c-1d4f-4a91-87ec-8c01204057b0
...
plot_sdf(sdf::DataFrame; nbins=(10,100,100,100),  
		 xlim=((0.0,10.0),(-10.0, 800.0), (0.0, 400.0), (0.0, 400.0)))

# ╔═╡ b0365dcc-2970-4ece-ad04-8860a66a5300
begin
	plot(step_hist(sdf.stepLength;
              nbins=100,
              xlim=(0.0, 400.0),
              logy=false,
              xlabel=" step length",
...
# ╔═╡ 6cb24aca-4a08-426a-92ae-c1c51da04970
begin
	hstl = hist1d(sdf.stepLength, 400)
end
...
md"""
- Cut away short steps (stepLength < 10)
"""
...
function save_stats(sdf2, vns, sample, field)
	hnst, xmnst, mxsth, max_sth, mxstl, max_stl = stats(sdf2, vns)
	...
	- #### steps:
	- centers = $(vect_to_fstr(hnst.centers, "%.2f"))
...
begin
	ss = save_stats(sdf2, vns, sample, field)
	Markdown.parse(ss)
end
