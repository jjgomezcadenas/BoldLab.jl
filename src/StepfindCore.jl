...
function A121_split_until_ready(dataX::Vector{<:Real}, N_iter::Int, demo::Float64)
    ...
    i_start = split_table[2, 1]
    i_stop = split_table[2, 2]
    i_next = split_table[2, 3]
    avl = split_table[2, 4]
    avr = split_table[2, 5]
...
function A1211_adapt_fit(FitX::Vector, split_table, idx::Int)
    ...
    if rank > 0
        istart = split_table[idx, 1]
        istop = split_table[idx, 3]
        FitX[istart:istop] .= split_table[idx, 4]
        FitX[istop+1:split_table[idx, 2]] .= split_table[idx, 5]
    end
    return FitX
end
